...
def getAbruptness(graph, label):
	...
	weighted_highest = sorted(weighted_abruptness, key=weighted_abruptness.get, reverse=True)[0]
...
def getBranchisess(graph, tau=0.1):
	...
	return np.mean(degree0), np.mean(degree1)
...
def getPitchChangeBetweenRules(graph, tau=0.75):
	...
	return np.mean(pitch_difference_between_rules), np.mean(freq_difference_between_rules)
...
def generateFeatures(graph, label):
	...
	d0, d1 = getAbruptness(graph, label)

	d2, d3 = getBranchisess(graph, 0.10)

	d4 = getRepeatedness(graph, 0.75)

	d5 = getMelodic(graph)

	d6, d7, d8, d9 = getPitchRange(graph)
	
	d10, d11 = getPitchChangeBetweenRules(graph, 0.75)
