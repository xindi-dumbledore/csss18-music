	...
	#for name in songs:
		...
		for i in jdata[name][1]:
			if len(jdata[name][1][i]) > 0:
				...
				if len(row) > len(mdata):
					mdata = row
					mrow = i

		if len(mdata) > 0:
			#mdata = thresholding(mrow)
			# Save the row with most notes
			try:
				print('Saving : {} {}'.format(name, str(mrow)))
			except:
				print('Saving')
			#print(os.path.join(sname, name.replace('/', ' ') + '_.csv'))
			genre.append(name.split(' ')[0])
			#saveData(os.path.join(sname, 'pop ' + name.replace('/', ' ').lower() + '_' + str(i) +' .csv'), mdata)
			saveData(os.path.join(sname, name.replace('/', ' ').lower() + '_' + str(i) +' .csv'), mdata)
	print(set(genre))
