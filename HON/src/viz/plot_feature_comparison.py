	...
	with open(fname, 'r') as f:
		...
		for row in reader:
			...
			if len(row[7][1:-1].split(',')) > 1:
				data['pitch_between_rules'].append(np.mean([float(r) for r in row[7][1:-1].split(',')]))
			else:
...
def plotDistribution(data, dtype, xname, sname, log=False):
	...
	pdata = [[v for v in data[n][dtype] if not np.isnan(v) and not np.isinf(v)] for n in names]
	...
	for i in range(0, len(pdata)):
		...
		plt.vlines(m, ymin=0, ymax=3, lw=1, color = sns.color_palette()[i])
...
if __name__ == '__main__':
	...
	data = {}
	data['American Folk'] = readFeature(os.path.join(dirname, 'folk_larger_aggregate.csv'))
	data['Rock'] = readFeature(os.path.join(dirname, 'rock_larger_aggregate.csv'))
	data['Classical'] = readFeature(os.path.join(dirname, 'classical_larger_aggregate.csv'))
	data['Jazz'] = readFeature(os.path.join(dirname, 'jazz_larger_aggregate.csv'))
	data['Pop'] = readFeature(os.path.join(dirname, 'pop_larger_aggregate.csv'))
	...
	plotDistribution(data, 'branchiness_mean', 'Branchiness (Mean)', sname, False)
